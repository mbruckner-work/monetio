...
class ModelBin:
    ...
    def readfile(self, filename, drange, verbose, century):
        ...
        if self.dset.variables:
            ...
            tvals = self.dset.t.values
            self.dset = self.dset.assign_coords(time=("t",tvals))
            self.dset = self.dset.reset_coords()
            self.dset = self.dset.set_coords(["time", "latitude", "longitude"])
            self.dset = reset_time_index(self.dset)
...
def combine_dataset(
    ...
    sourcelist = []
    enslist = []
    #for mmm, key in enumerate(blist):
    #    # fname = val[0]
    for iii, bbb in enumerate(blist):
            fname = bbb[0]
            ...
            enslist.append(bbb[1])
            sourcelist.append(bbb[2])
            ...
            if iii == 0:
                stime = tvals[0]
                xnew = reset_time_index(xrash,stime=stime)
                # start time for indexing will always be earliest time in first file in list.
                xrash = reset_time_index(xrash,stime=stime)
            else:
                xrash = reset_time_index(xrash,stime=stime)
                ccc, xnew = xr.align(xrash, xnew, join="outer")
            xlist.append(xrash)
    ylist = []
    slist = []
    hlist = []
    for iii, temp in enumerate(xlist):
        aaa, bbb = xr.align(temp, xnew, join="outer")
        aaa = reset_time_coord(aaa)
        aaa.expand_dims("ens")
        aaa["ens"] = enslist[iii]
        aaa.expand_dims("source")
        aaa["source"] = sourcelist[iii]
        hlist.append(aaa)
    # concat along the 'ens' axis
    #newhxr = xr.concat(hlist, "source")
    sourcelist = list(set(sourcelist))
    enslist = list(set(enslist))
    if len(sourcelist)==1:
        newhxr = xr.concat(hlist,dim='ens')
    elif len(enslist)==1:
        newhxr = xr.concat(hlist,dim='source')
    else:
        newhxr = xr.concat(hlist,dim='source')
    ...
    #newhxr = reset_time_coord(newhxr)
    ...
    keylist = ["coordinate start time",'coordinate time dt (hours)']
    for key in keylist:
        if key in xnew.attrs.keys():
           newhxr = newhxr.assign_attrs({key:xnew.attrs[key]})
...
def reset_time_index(hxr,stime=None,dt=1):
    """
    ...
    temp = hxr.assign_coords(t=('t',ilist))
    atthash = {'coordinate start time': stime}
    atthash['coordinate time dt (hours)'] = dt
    temp.attrs.update(atthash)
